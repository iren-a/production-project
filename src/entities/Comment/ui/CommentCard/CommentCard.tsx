    ...
    <ToggleFeatures
      ...
      on={
        <Card padding="24" fullWidth>
          <VStack data-testid="CommentCard.Content" gap="8" fullWidth className={className}>
      ...
      off={
        ...
        >
          <AppLinkDeprecated to={getRouteProfile(comment.user.id)} className={cls.header}>
            {comment.user.avatar && (
              <AvatarDeprecated size={30} className={cls.avatar} src={comment.user.avatar} />
            )}
            <TextDeprecated title={comment.user.username} />
          </AppLinkDeprecated>
          <TextDeprecated data-testid="Content" text={comment.text} />
        </VStack>
      }
    />
  );
